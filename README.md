...
level code. If the config section specifies more deployments in the **$PUSHY_TRIGGERS** variable these will be run after the main deployment has finished
it's execution.

GitPushy Config Script
--------
The Config stage is intended to set key variables necessary for making a pushy deployment possible. The most simple version of this script consists of
setting the **$PUSHY_DEPLOY_DIR** variable to the place where your final code will reside. Without specifying this directory nothing will be done to deploy
...
If this process is rather strenuous and time consuming it may be better to run these detached from a *.gitpushy-custom* script to avoid
...
litmus tests to prove the application is stable and ready to be deployed. If at any time you cause the stage section to exit with a value other than zero
GitPushy will abort at the end of that section and display the error code to the committer in the git commit scroll-back. If you need to force immediate
...
preferable to do all of the heavy testing for success in the stage section to insure success when deploy is called.
...
--------
In some cases of more complex deployments you may wish to have more control over when and how deployments happen. In these case, GitPushy will replace
...
There are many scenarios that lend itself to custom scripting. Suppose a Makefile needs to be run for a long period, you want to add lock checking to
...
--------
When writing advanced deployments the following commands can be useful to sculpt your deployment process. They are only available in the config and
...
deployments of another. This is used internally to deploy the committed branches like normal. This will trigger a fresh deployment process of the branch.
...
* **gitpushy-push-replicate {hostname} [ {hostname} ...]** - This is meant to be run in the build stage where each replication is put in its own branch with the
...
there too. You can prevent continuous replication by testing:
...
--------
When *.gitpushy-custom* is not desired or if you wish to avoid shipping the custom script with the repository for security reasons you can put
...
is not contained within the version control system it can rendered inoperable if changed or deleted. You may want to keep a separate repository with branch hooks for your repositories.
